...
def load_all_historical_data() -> str:
    ...
    try:
        with sqlite3.connect(DB) as con:
            # 1. Pull core draft records and classifications
            df = pd.read_sql_query("SELECT * FROM draft_roi_scores", con)
            
            # Format clean strings or fallbacks safely 
            ...
            # Calculate mock/real aggregate metric cards grouped by manager
...
<head>
  ...
  <!-- Tailwind for native UI layout construction -->
  <script src="https://jsdelivr.net"></script>
  <!-- Vue 3 CDN for lightning fast mobile client interactivity -->
...
<body>
  <div id="app" v-cloak>
    ...
    <div class="w-full max-w-[420px] mx-auto text-[#E4E4E6] p-4 pb-12 select-none">
      ...
      <!-- Main Navigation Functional Tabs -->
      ...
      <!-- VIEW 1: DYNAMIC LEAGUE HISTORY VIEW WITH DRAFT GRADES -->
      <div v-if="currentView === 'history'">
        ...
        <!-- Automated Calculated Draft Cards -->
        <section class="bg-[#1C1C1E] rounded-xl p-4 border border-[#242426] mb-4 shadow-xl">
          <header class="flex justify-between items-start mb-4">
            <div>
              ...
              <h3 class="text-[36px] font-black text-white tracking-tight leading-none transition-all">{{{{ calculatedGrade }}}</h3>
            </div>
          ...
          <!-- Rendered Trend Line Matrix inside the frame container -->
